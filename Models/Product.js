const mongoose = require("mongoose");

const productSchema = new mongoose.Schema(
  {
    name: { type: String, trim: true },
    slug: { type: String, trim: true },
    short_desc: { type: String, trim: true },
    long_desc: { type: String, trim: true },
    additional_info: { type: String, trim: true },
    ship_policy: { type: String, trim: true },
    price: { type: Number },
    sale_price: { type: Number },
    sku: { type: Number },
    Collection_Name: { type: String },
    installation_method: { type: String },
    thickness: { type: String },
    wear_layer_thickness: { type: String },
    width: { type: String },
    length: { type: String },
    review: { type: Number },
    ratings: { type: Number },
    stock: { type: Number },
    BestSeller: { type: Boolean },
    newProduct: { type: Boolean },
    IsmostViewed: { type: Boolean },
    IsmostViewed: { type: Boolean },
    isDiscount: { type: Boolean },
    sold: { type: Number },
    SuperCatID: { type: String },
    SuperCatName: { type: String },
    CatID: { type: String },
    CatName: { type: String },
    SubCatID: { type: String },
    SubCatName: { type: String },
    BrandID: { type: String },
    BrandName: { type: String },
    BoxPrice: { type: Number },
    BoxCoverage: { type: Number },
    isAddon: { type: Boolean, default: false },
    productPictures: [{ type: String }],
    color: { type: String },
    updatedAt: { type: Date },
    Manufacture: {type:String},
    Application: {type:String},
    Product_Type: {type:String},
    Commercial_Warranty: {type:String},
    Certifation: {type:String},
    Core_Type_Detail: {type:String},
    Years: {type:Number},
    Edge_Type: {type:String},
    Coverage_sqft: {type:Number},
    length_cm: {type:String},
    Format_in: {type:String},
    Manufacturer_color: {type:String},
    length_in_fraction: {type:Number},
    Shape: {type:String},
    Recidential_warranty: {type:String},
    Underpad_attached: {type:Number},
  },
  { timestamps: true }
);

module.exports = mongoose.model("Product", productSchema);
